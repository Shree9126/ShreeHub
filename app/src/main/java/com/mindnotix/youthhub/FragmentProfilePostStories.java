...
public class FragmentProfilePostStories extends Fragment implements IBaseResponseListener, SwipeRefreshLayout.OnRefreshListener {

    private static final String TAG = FragmentProfilePostStories.class.getSimpleName();
    ...
    void showPopupWindowMypost(final View view, final int position) {
        ...
        } else if (dataArrayList.get(position).getPosts().getPmStep().equals("0") &&
                dataArrayList.get(position).getPosts().getPmTimeline().equals("1")) {
            popup.getMenu().getItem(2).setTitle("Add to step");
            popup.getMenu().getItem(2).setIcon(R.drawable.ic_add_black_24dp);
        }
        if (dataArrayList.get(position).getPostUserActivity().getIs_favour_user().equals("1"))
